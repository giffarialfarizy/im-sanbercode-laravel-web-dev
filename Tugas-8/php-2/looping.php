...
<body>
  ...
  */
  ...
  // Output:
  $items_assoc = [];
  echo "Masing-masing element array asosiatif (sesuai soal): <br>";
  foreach ($items as $item) {
    $new_item["id"] = $item[0];
    $new_item["name"] = $item[1];
    $new_item["price"] = $item[2];
    $new_item["description"] = $item[3];
    $new_item["source"] = $item[4];
    array_push($items_assoc, $new_item);

    print_r($new_item);
    echo "<br>";
  }

  echo "<br>Seluruh array asosiatif: <br>";
  print_r($items_assoc);
